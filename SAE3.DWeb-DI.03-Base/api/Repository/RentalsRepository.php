...
class RentalsRepository extends EntityRepository {
    ...
    public function rentalByCountry(): array {
        $requete = $this->cnx->prepare("
            SELECT Customers.country, SUM(Rentals.rental_price) as total_rentals FROM Rentals JOIN Customers ON Rentals.customer_id = Customers.id GROUP BY Customers.country;
        ");
        $requete->execute();
